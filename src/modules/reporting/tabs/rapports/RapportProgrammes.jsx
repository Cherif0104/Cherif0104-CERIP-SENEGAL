...
export default function RapportProgrammes() {
  ...
  const generateRapport = async () => {
    setLoading(true)
    try {
      const { data: rapportData, error } = await reportingService.getRapportProgrammes(filters)
      if (error) {
        logger.error('RAPPORT_PROGRAMMES', 'Erreur génération', error)
        alert('Erreur lors de la génération du rapport')
        return
      }
      setData(rapportData || [])
      logger.info('RAPPORT_PROGRAMMES', `Rapport généré: ${rapportData?.length || 0} programmes`)
    } catch (error) {
      logger.error('RAPPORT_PROGRAMMES', 'Erreur inattendue', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleExportExcel = () => {
    const columns = [
      ...
      { key: 'nombre_projets', label: 'Nb Projets' },
      { key: 'nombre_beneficiaires', label: 'Nb Bénéficiaires' },
    ]
    const formattedData = formatDataForExport(data, columns)
    exportToExcel(formattedData, 'rapport-programmes.csv')
  }
    ...
    <div className="rapport-base">
      ...
      {!loading && data.length > 0 && (
        <div className="rapport-results">
          <h3>Résultats ({data.length} programmes)</h3>
          <DataTable columns={columns} data={data} />
        </div>
      )}

      {!loading && data.length === 0 && filters.dateDebut && (
        <div className="rapport-empty">
          <p>Cliquez sur "Générer le rapport" pour afficher les données</p>
        </div>
      )}
    </div>
  )
