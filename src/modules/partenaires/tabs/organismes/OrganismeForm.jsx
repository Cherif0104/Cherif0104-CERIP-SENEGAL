...
import './OrganismeForm.css'

export default function OrganismeForm({ id, onClose }) {
  const isEdit = !!id
  ...
  const loadOrganisme = async () => {
    ...
    try {
      ...
      if (error) {
        logger.error('ORGANISME_FORM', 'Erreur chargement organisme', error)
      ...
      logger.error('ORGANISME_FORM', 'Erreur inattendue', error)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const dataToSave = {
        ...formData,
        contacts: formData.contacts.filter((c) => c.nom || c.email), // Filtrer contacts vides
      }

      if (isEdit) {
        const { error } = await organismesService.update(id, dataToSave)
        if (error) {
          setError(error.message || 'Erreur lors de la mise à jour')
          return
        }
        logger.info('ORGANISME_FORM', 'Organisme mis à jour', { id })
      } else {
        const { error } = await organismesService.create(dataToSave)
        if (error) {
          setError(error.message || 'Erreur lors de la création')
          return
        }
        logger.info('ORGANISME_FORM', 'Organisme créé')
      }
    ...
    <div className="organisme-form-page">
      <div className="organisme-form-container">
        <div className="organisme-form-header">
          ...
          <h1>{isEdit ? 'Modifier l\'organisme' : 'Nouvel organisme international'}</h1>
        </div>
