...
export default function StructureForm({ id, onClose }) {
  const isEdit = !!id
  ...
  const loadStructure = async () => {
    ...
    try {
      ...
      if (error) {
        logger.error('STRUCTURE_FORM', 'Erreur chargement structure', error)
      ...
      logger.error('STRUCTURE_FORM', 'Erreur inattendue', error)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        const { error } = await structuresService.update(id, dataToSave)
        if (error) {
          setError(error.message || 'Erreur lors de la mise à jour')
          return
        }
        logger.info('STRUCTURE_FORM', 'Structure mise à jour', { id })
      } else {
        const { error } = await structuresService.create(dataToSave)
        if (error) {
          setError(error.message || 'Erreur lors de la création')
          return
        }
        logger.info('STRUCTURE_FORM', 'Structure créée')
      }
          ...
          <div className="form-row">
            <Select
              ...
              options={[
                ...
                { value: 'GIE', label: 'GIE (Groupement d\'Intérêt Economique)' },
