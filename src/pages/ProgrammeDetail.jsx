...
export default function ProgrammeDetail() {
  ...
  const loadAll = async () => {
    setLoading(true)
    await Promise.all([loadProgramme(), loadProjets(), loadStats(), loadEligibilite()])
    setLoading(false)
  }
  ...
  const loadProjets = async () => {
    try {
      ...
      if (error) {
        console.error('Erreur lors du chargement des projets du programme', error)
      } else {
        setProjets(data || [])
      }
    } catch (error) {
      console.error('Erreur inattendue lors du chargement des projets du programme', error)
    }
  }
  ...
  const loadStats = async () => {
    try {
      ...
      const candidatsCount = (candidatsRes.data || []).filter(c =>
        c.appel_id && projetsIds.includes(c.projet_id)
      ).length
