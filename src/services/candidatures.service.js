...
import { notificationsCandidatService } from './notifications-candidat.service'
...
export const candidaturesService = {
  ...
  async updateStatutEligibilite(id, statutEligibilite, motif = null) {
    try {
      ...
      const { data: oldCandidat } = await candidatRepository.findById(id)
      const ancienStatut = oldCandidat?.statut_eligibilite
      ...
      // Notifier le candidat du changement de statut (non-bloquant)
      if (ancienStatut !== statutEligibilite) {
        try {
          await notificationsCandidatService.notifyStatutChange(id, ancienStatut, statutEligibilite)
        } catch (notifError) {
          logger.warn('CANDIDATURES_SERVICE', 'Erreur notification (non-bloquant)', notifError)
        }
      }
